...
export const scraperApi = {
  ...
  createJob:  (payload: CreateScraperJobRequest) => api.post('/scraper/jobs', payload).then(unwrap),
...
export const analyticsApi = {
  profile:      (payload: ProfileRequest)          => api.post('/analytics/profile', payload).then(unwrap),
  chart:        (payload: ChartRequest)             => api.post('/analytics/chart', payload).then(unwrap),
  ...
  sql:          (payload: SQLQueryRequest)          => api.post('/analytics/sql', payload).then(unwrap),
}

export const pipelinesApi = {
  list:   ()                                    => api.get('/pipelines/').then(unwrap),
  create: (payload: CreatePipelineRequest)      => api.post('/pipelines/', payload).then(unwrap),
  update: (id: string, payload: Partial<CreatePipelineRequest>) => api.put(`/pipelines/${id}`, payload).then(unwrap),
...
export const aiApi = {
  ...
  train:      (payload: TrainModelRequest) => api.post('/ai/train', payload).then(unwrap),
  predict:    (payload: PredictRequest)    => api.post('/ai/predict', payload).then(unwrap),
  forecast:   (payload: ForecastRequest)   => api.post('/ai/forecast', payload).then(unwrap),
  cluster:    (payload: Record<string, unknown>) => api.post('/ai/cluster', payload).then(unwrap),
}

export const exportsApi = {
  create:  (payload: ExportRequest)           => api.post('/exports/', payload).then(unwrap),
