...
import {
  Table2, Upload, Download, Filter, Plus, Search, Zap, Database, Code2
} from 'lucide-react'
import { clsx } from 'clsx'
import { useDropzone } from 'react-dropzone'
import { useDatasets, useDataset, useUploadDataset } from '../hooks/useApi'
import { analyticsApi } from '../lib/api'
import { Badge, Panel, Empty, Spinner, Modal, Tabs, StatCard } from '../components/ui'
...
export function DataTablesPage() {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row overflow-hidden bg-[var(--bg)]">
      ...
      <div data-tour="dataset-list" className="w-full md:w-64 shrink-0 border-r border-[var(--surface-border)] flex flex-col bg-[var(--surface)] overflow-hidden max-h-[35vh] md:max-h-none">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--surface-border)] shrink-0">
          ...
          <button data-tour="dataset-upload" className="btn-primary text-xs py-1 px-2.5" onClick={() => setShowUpload(true)}>
            <Plus size={11}/> Add
          </button>
        </div>
      ...
      {/* Main Content */}
      <div data-tour="dataset-main" className="flex-1 flex flex-col overflow-hidden">
        {!selectedId ? (
