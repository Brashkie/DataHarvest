...
import { Brain, Play, TrendingUp, Cpu, Layers, Plus, Zap } from 'lucide-react'
import { useModels, useTrainModel } from '../hooks/useApi'
import type { TrainModelRequest } from '../lib/types.generated'
import { Badge, Empty, Spinner, Modal, Tabs, StatCard } from '../components/ui'
...
export function AIStudioPage() {
  ...
  const handleTrain = async () => {
    ...
    await trainModel.mutateAsync(trainForm as TrainModelRequest)
    setShowTrain(false)
  }

  return (
    <div className="h-full flex flex-col overflow-hidden bg-[var(--bg)]">
      <div data-tour="ai-stats" className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-5 pb-3 shrink-0">
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-4 px-5 pb-5 overflow-hidden min-h-0">
        {/* Models List */}
        <div data-tour="models-list" className="w-full md:w-72 shrink-0 panel flex flex-col overflow-hidden max-h-[40vh] md:max-h-none">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--surface-border)] shrink-0">
            ...
            <button data-tour="train-btn" className="btn-primary text-xs py-1 px-2.5" onClick={() => setShowTrain(true)}>
              <Plus size={11}/> Train
            </button>
          </div>
        ...
        <div data-tour="ai-tabs" className="flex-1 panel overflow-hidden flex flex-col">
          <Tabs
            tabs={[
