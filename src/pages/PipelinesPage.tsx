...
import {
  GitBranch, Play, Plus, CheckCircle2, XCircle, Zap, Globe, Database,
...
import { StatusBadge, StatCard, Panel, Empty, Spinner, Modal, Tabs } from '../components/ui'
...
export function PipelinesPage() {
  ...
  const handleCreate = async () => {
    ...
    await createPipeline.mutateAsync({
      ...
      definition: {
        nodes: nodes.map(n => ({
          id: n.id,
          type: (n.data?.nodeType as string | undefined) ?? 'transform',
  ...
  return (
    <div className="h-full flex flex-col md:flex-row overflow-hidden bg-[var(--bg)]">
      {/* Left Sidebar ── Pipeline List */}
      <div data-tour="pipeline-list" className="w-full md:w-64 shrink-0 border-r border-[var(--surface-border)] flex flex-col bg-[var(--surface)] overflow-hidden max-h-[30vh] md:max-h-none">
        {/* Stats */}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[var(--surface-border)] bg-[var(--surface)] shrink-0">
          <div data-tour="node-palette" className="flex gap-1">
        ...
        {/* Content */}
        <div data-tour="pipeline-canvas" className="flex-1 overflow-hidden">
          {tab === 'editor' && (
            <ReactFlow
              nodes={nodes}
              edges={edges}
